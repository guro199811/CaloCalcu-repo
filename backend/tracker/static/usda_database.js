...
document.addEventListener('DOMContentLoaded', function () {
    ...
    totalCaloriesDiv.id = 'totalCalories';
    ...
    function updateTotalCalories() {
        const scaleTableRows = document.querySelectorAll('#scaleTable tbody tr');
        let totalCalories = 0;

        scaleTableRows.forEach(row => {
            const caloriesMatch = row.innerHTML.match(/<td>Energy: (\d+(\.\d+)?) kcal<\/td>/);
            if (caloriesMatch) {
                totalCalories += parseFloat(caloriesMatch[1]);
            }
        });

        totalCaloriesDiv.textContent = `Total Calories on Scales: ${totalCalories.toFixed(2)} kcal`;

        // Check if the totalCaloriesDiv is already added, if not, append it
        if (!document.getElementById('totalCalories')) {
            scaleTable.insertAdjacentElement('afterend', totalCaloriesDiv);
        }
    }
